...
SOURCE_CHROME_PROFILE="${FINMIND_SOURCE_CHROME_PROFILE:-$HOME/Library/Application Support/Google/Chrome}"
RENDER_PROFILE_DIR="${FINMIND_RENDER_PROFILE_DIR:-/tmp/render-proof-chrome-profile}"
...
COMMIT_SHA="${FINMIND_RENDER_PROOF_SHA:-f5667cd}"
...
if [ ! -d "$RENDER_PROFILE_DIR/Default" ]; then
  rm -rf "$RENDER_PROFILE_DIR"
  mkdir -p "$RENDER_PROFILE_DIR"
  cp -R "$SOURCE_CHROME_PROFILE/." "$RENDER_PROFILE_DIR/"
fi
